# ProxyLite

## 中文

ProxyLite 规则集。

### 规则统计

- 总数: `982`
- `DOMAIN-SUFFIX`: 843
- `DOMAIN`: 23
- `DOMAIN-KEYWORD`: 20
- `USER-AGENT`: 6
- `IP-CIDR`: 86
- `IP-CIDR6`: 4

### 使用说明

- 客户端: `Clash`
- 文件: `rule/Clash/ProxyLite/ProxyLite.yaml`
- 直链: `https://raw.githubusercontent.com/lylywayr/proxy-rulesets/main/rule/Clash/ProxyLite/ProxyLite.yaml`
- 已省略的不兼容类型: 无

### 数据来源

- blackmatrix7 ProxyLite: https://raw.githubusercontent.com/blackmatrix7/ios_rule_script/master/rule/Surge/ProxyLite/ProxyLite.list

## English

ProxyLite rule set.

- Client: `Clash`
- File: `rule/Clash/ProxyLite/ProxyLite.yaml`
- Raw URL: `https://raw.githubusercontent.com/lylywayr/proxy-rulesets/main/rule/Clash/ProxyLite/ProxyLite.yaml`
- Omitted unsupported types: `无`
